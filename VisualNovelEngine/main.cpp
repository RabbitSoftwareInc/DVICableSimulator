...
{
	std::wstring game = game_info.game_name;
...
std::vector<ScenarioTexture_t> load_textures(std::wstring directory)
{
	...
	while (FindNextFileW(hFind, &findData) != 0)
	{
		if (get_file_ext(std::wstring(findData.cFileName)) == L".png")
		{
			...
			//texture.texture_data.setSmooth(true);
#endif
...
std::vector<ScenarioDialogueScene_t> load_scenes(std::wstring path)
{
	...
	if (wcsstr(GetCommandLineW(), L"-skid1337"))
	{
		...
		scene.person1.talking = true;
		scene.person1.talking_text = L"Welcome to " + game_info.game_name + L"!";
...
void audio_settings_menu()
{
	...
	ImGui::Begin(LANG(L"Audio settings", L"Íàñòðîéêè çâóêà"), (bool*)0, flags);

	ImGui::Text(LANG(L"Music volume", L"Ãðîìêîñòü ìóçûêè"));
	ImGui::SliderInt("##MUSICVOL", &new_settings.music_volume, 0, 100);

	ImGui::Text(LANG(L"Sound volume", L"Ãðîìêîñòü çâóêîâ"));
	ImGui::SliderInt("##SOUNDVOL", &new_settings.sound_volume, 0, 100);

	if (ImGui::Button(LANG(L"Apply", L"Ïðèìåíèòü")))
	{
	...
	if (ImGui::Button(LANG(L"Cancel", L"Îòìåíà")))
		new_settings = audio_settings;

	ImGui::SameLine();

	if (ImGui::Button(LANG(L"Close", L"Çàêðûòü")))
		audio_settings_open = false;
...
void game_settings_menu()
{
	...
	ImGui::Begin(LANG(L"Game settings", L"Íàñòðîéêè èãðû"), (bool*)0, flags);

	ImGui::Checkbox(LANG(L"Auto save", L"Àâòîñîõðàíåíèå"), &new_settings.auto_save);
	ImGui::Checkbox(LANG(L"Show FPS counter", L"Ïîêàçûâàòü ñ÷åò÷èê FPS"), &new_settings.show_fps_counter);

	ImGui::Checkbox(LANG(L"Animated dialogue text", L"Àíèìèðîâàííûé òåêñò äèàëîãîâ"), &new_settings.animated_dialogue_text);

	ImGui::Text(LANG(L"Text animation speed", L"Ñêîðîñòü àíèìàöèè òåêñòà"));
	ImGui::SliderInt("##Text animation speed", &new_settings.text_animation_speed, 1, 100);

	ImGui::Text(LANG(L"Menu language", L"ßçûê ìåíþ"));
...
void video_settings_menu()
{
	...
	ImGui::Begin(LANG(L"Video settings", L"Íàñòðîéêè âèäåî"), (bool*)0, flags);

	ImGui::Text(LANG(L"Render mode", L"Ðåæèì ðåíäåðèíãà"));
	ImGui::Combo("##Render mode", &new_settings.render_mode, RenderMode, ARRAYSIZE(RenderMode));

	ImGui::Text(LANG(L"Window mode", L"Ðåæèì îêíà"));
	ImGui::Combo("##Window mode", &new_settings.screen_mode, ScreenMode, ARRAYSIZE(ScreenMode));

	ImGui::Checkbox(LANG(L"VSync", L"Âåðò. ñèíõðîíèçàöèÿ"), &new_settings.vsync);

	ImGui::Text(LANG(L"Texture quality", L"Êà÷åñòâî òåêñòóð"));
	...
	if (ImGui::Button(LANG(L"Apply", L"Ïðèìåíèòü")))
	{
		...
		if (video_settings.screen_mode != 0)
			MessageBoxW(GetForegroundWindow(), LANG_W(L"Restart game to apply settings", L"Ïåðåçàïóñòèòå èãðó äëÿ ïðèìåíåíèÿ íàñòðîåê"), LANG_W(L"Success", L"Óñïåøíî"), 0);
	}
...
void save_menu(Scenario_t& scenario)
{
	...
	ImGui::Text(LANG(L"Enter save name", L"Ââåäèòå èìÿ ñîõðàíåíèÿ"));

	ImGui::SetNextItemWidth(240);
	ImGui::InputText("##Save name input", save_name, 260);

	if (ImGui::Button(LANG(L"Save", L"Ñîõðàíèòü"), ImVec2(240, 20)))
	{
...
void main_game_menu(Scenario_t& scenario)
{
	...
	if (ImGui::Button(LANG(L"Back to game", L"Âåðíóòüñÿ ê èãðå"), ImVec2(230, 20)))
	{
		game_menu_open = false;
		save_menu_open = false;
	}
	...
	if (!scenario_editor)
	{
		if (ImGui::Button(LANG(L"Save game", L"Ñîõðàíèòü èãðó"), ImVec2(230, 20)))
	...
	if (ImGui::Button(LANG(L"Video settings", L"Íàñòðîéêè âèäåî"), ImVec2(230, 20)))
		video_settings_open = !video_settings_open;

	if (ImGui::Button(LANG(L"Audio settings", L"Íàñòðîéêè çâóêà"), ImVec2(230, 20)))
		audio_settings_open = !audio_settings_open;

	if (ImGui::Button(LANG(L"Game settings", L"Íàñòðîéêè èãðû"), ImVec2(230, 20)))
	...
	if (ImGui::Button(LANG(L"Exit to main menu", L"Âûéòè â ãëàâíîå ìåíþ"), ImVec2(230, 20)))
		exit_to_main_menu(false);

	if (ImGui::Button(LANG(L"Quit game", L"Âûõîä èç èãðû"), ImVec2(230, 20)))
		exit(0);
	...
	if (history_size.x != FLT_MAX && history_size.y != FLT_MAX)
	{
		...
		ImGui::Text(LANG(L"History", L"Èñòîðèÿ"));
...
void texture_selector(const wchar_t* name, Scenario_t& scenario, std::wstring& texture)
{
	...
	ImGui::InputTextWithHint(std::string(std::string("##SEARCH") + ws2s(name)).c_str(), LANG(L"Search", L"Ïîèñê"), &search_buf);
...
void scenario_selector(const wchar_t* name, std::wstring& scenario, int idx = 0)
{
	...
	ImGui::SetNextItemWidth(200);
	ImGui::InputTextWithHint(std::string(std::string("##SEARCH") + ws2s(name) + std::to_string(idx)).c_str(), LANG(L"Search", L"Ïîèñê"), &search_buf);
...
void main_game()
{
	...
	if (scenario_editor)
	{
		...
		ImGui::Begin(LANG(L"Scenario", L"Ñöåíàðèé"), (bool*)0, ImGuiWindowFlags_::ImGuiWindowFlags_NoResize | ImGuiWindowFlags_::ImGuiWindowFlags_NoCollapse);

		ImGui::Text(LANG(L"Scenes", L"Ñöåíû"));
		...
		if (ImGui::Button(LANG(L"Create new scene", L"Ñîçäàòü íîâóþ ñöåíó"), ImVec2(230, 20)))
		{
			ScenarioDialogueScene_t scene;
		...
		if (ImGui::Button(LANG(L"Create scene duplicate", L"Ñîçäàòü äóáëèêàò ñöåíû"), ImVec2(230, 20)))
		{
		...
		if (ImGui::Button(LANG(L"Edit scene", L"Ðåäàêòèðîâàòü ñöåíó"), ImVec2(230, 20)))
			editing_scene = true;
		...
		if (ImGui::Button(LANG(L"Delete scene", L"Óäàëèòü ñöåíó"), ImVec2(230, 20)))
		{
		...
		if (ImGui::Button(LANG(L"Save scenario", L"Ñîõðàíèòü ñöåíàðèé"), ImVec2(230, 20)))
		...
		if (editing_scene)
		{
			...
			ImGui::Begin(LANG(L"Scene editor", L"Ðåäàêòîð ñöåíû"), &editing_scene, ImGuiWindowFlags_::ImGuiWindowFlags_NoResize | ImGuiWindowFlags_::ImGuiWindowFlags_NoCollapse);

			if (ImGui::BeginTabBar("##SCENEEDITORTABS"))
			{
				if (ImGui::BeginTabItem(LANG(L"Main", L"Îñíîâíîå")))
				{
					if (advanced_scenes)
						ImGui::Columns(6);
					else
					...
					ImGui::Text(LANG(L"Main character text", L"Òåêñò ãëàâíîãî ïåðñîíàæà"));
					ImGui::SetNextItemWidth(200);
					...
					ImGui::NextColumn();
					texture_selector(LANG_W(L"Background texture", L"Ôîíîâàÿ òåêñòóðà"), scenario, scene.background_texture);

					if (advanced_scenes)
					{
						ImGui::NextColumn();
						texture_selector(LANG_W(L"Background overlay texture", L"Íàëîæåííàÿ ôîíîâàÿ òåêñòóðà"), scenario, scene.background_overlay_texture);
					}

					ImGui::NextColumn();
					sound_selector(LANG_W(L"Background music", L"Ôîíîâàÿ ìóçûêà"), scenario, scene.background_music);

					ImGui::NextColumn();
					sound_selector(LANG_W(L"Additional sound", L"Äîïîëíèòåëüíûé çâóê"), scenario, scene.additional_scene_sound);

					if (advanced_scenes)
					{
						ImGui::NextColumn();
						texture_selector(LANG_W(L"Screen overlay texture", L"Íàëîæåííàÿ òåêñòóðà"), scenario, scene.overlay_texture);
					}
				...
				if (ImGui::BeginTabItem(LANG(L"Character 1", L"Ïåðñîíàæ 1")))
				{
					...
					ImGui::Text(LANG(L"Character name", L"Èìÿ ïåðñîíàæà"));
					...
					ImGui::Text(LANG(L"Character text", L"Òåêñò ïåðñîíàæà"));
					...
					{
						ImGui::NextColumn();
						texture_selector(LANG_W(L"Left body texture", L"Òåêñòóðà ëåâîé ÷àñòè òåëà"), scenario, person.person_texture_left);

						ImGui::NextColumn();
						texture_selector(LANG_W(L"Right body texture", L"Òåêñòóðà ïðàâîé ÷àñòè òåëà"), scenario, person.person_texture_right);

						ImGui::NextColumn();
						texture_selector(LANG_W(L"Head texture", L"Òåêñòóðà ãîëîâû"), scenario, person.person_texture_head);
					}
					else
					{
						ImGui::NextColumn();
						texture_selector(LANG_W(L"Body texture", L"Òåêñòóðà òåëà"), scenario, person.person_texture);
					}
				...
				if (ImGui::BeginTabItem(LANG(L"Character 2", L"Ïåðñîíàæ 2")))
				{
				...
				if (ImGui::BeginTabItem(LANG(L"Character 3", L"Ïåðñîíàæ 3")))
				{
				...
				if (ImGui::BeginTabItem(LANG(L"Character 4", L"Ïåðñîíàæ 4")))
				{
				...
				if (ImGui::BeginTabItem(LANG(L"Buttons", L"Êíîïêè")))
				{
					ImGui::Columns(4);

					{
						...
						ImGui::Text(LANG(L"Button name", L"Íàçâàíèå êíîïêè"));
						...
						scenario_selector(LANG_W(L"Scenario to load", L"Ñöåíàðèé äëÿ çàãðóçêè"), button.button_scenario_to_load, 1);
					}
					ImGui::NextColumn();

					{
						...
						scenario_selector(LANG_W(L"Scenario to load", L"Ñöåíàðèé äëÿ çàãðóçêè"), button.button_scenario_to_load, 2);
					}
					ImGui::NextColumn();

					{
						...
						scenario_selector(LANG_W(L"Scenario to load", L"Ñöåíàðèé äëÿ çàãðóçêè"), button.button_scenario_to_load, 3);
					}
					ImGui::NextColumn();

					{
						...
						scenario_selector(LANG_W(L"Scenario to load", L"Ñöåíàðèé äëÿ çàãðóçêè"), button.button_scenario_to_load, 4);
					}
...
void game_render()
{
	...
	if (!game_started)
	{
		...
		if (select_scenario)
		{
			...
			ImGui::Text(LANG(L"Select scenario", L"Âûáåðèòå ñöåíàðèé"));
			...
			if (scenario_editor)
			{
				...
				ImGui::Text(LANG(L"Enter new scenario name", L"Ââåäèòå íàçâàíèå íîâîãî ñöåíàðèÿ"));

				ImGui::SetNextItemWidth(230);
				ImGui::InputText("##Scenario name input", scenario_name, 64);

				if (ImGui::Button(LANG(L"Create new scenario", L"Ñîçäàòü íîâûé ñöåíàðèé"), ImVec2(230, 20)))
				{
			...
			if (selected_scenario > -1)
			{
				...
				ImGui::Text(LANG(L"Enter player name", L"Ââåäèòå èìÿ èãðîêà"));
				...
				{
					if (ImGui::Button(scenario_editor ? LANG(L"Edit", L"Ðåäàêòèðîâàòü") : LANG(L"Play", L"Èãðàòü"), ImVec2(230, 20)))
					{
						wchar_t WBuf[32];
		...
		else if (select_save)
		{
			...
			ImGui::Text(LANG(L"Select save", L"Âûáåðèòå ñîõðàíåíèå"));
			...
			if (ImGui::Button(LANG(L"Refresh list", L"Îáíîâèòü ñïèñîê"), ImVec2(230, 20)))
			{
				saves = get_directory_files_name_w(L".\\game\\saves", L".savegame");
				save_names = get_directory_files_name(".\\game\\saves", ".savegame");
			}
			...
			if (selected_save > -1)
			{
				...
				if (ImGui::Button(LANG(L"Play", L"Èãðàòü"), ImVec2(230, 20)))
				{
					GameSave_t save;
					...
					if (load_save(saves.at(selected_save), save) && find_scenario_index(save.scenario_name) != -1)
					{
						...
						MessageBoxW(GetForegroundWindow(), LANG_W(L"Failed to load save", L"Íå óäàëîñü çàãðóçèòü ñîõðàíåíèå"), LANG_W(L"Error", L"Îøèáêà"), 0);
				}
			}
		}
		else
		{
			if (ImGui::Button(scenario_editor ? LANG(L"Edit scenario", L"Ðåäàêòèðîâàòü ñöåíàðèé") : LANG(L"Start new game", L"Íà÷àòü íîâóþ èãðó"), ImVec2(230, 20)))
				select_scenario = true;

			if (!scenario_editor)
			{
				if (ImGui::Button(LANG(L"Load save", L"Çàãðóçèòü ñîõðàíåíèå"), ImVec2(230, 20)))
					select_save = true;
			}
...
RENDER_INIT:
	...
	ImGui::CreateContext();
#else
	sf::ContextSettings window_settings = sf::ContextSettings();
	window_settings.antialiasingLevel = 4;

	sf::RenderWindow window(sf::VideoMode(video_settings.screen_width, video_settings.screen_height), std::wstring(game_info.game_name + L" (" + game_renderer + L")").c_str(), sf::Style::Titlebar | sf::Style::Close | (video_settings.screen_mode == 0 ? sf::Style::Fullscreen : 0), window_settings);
